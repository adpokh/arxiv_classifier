...
По описанию задания надо было взять для начала модель distilbert-base-cased (я взяла uncased, т.к. нам лучше не учитывать регистр букв), и эта модель сразу дала очень хороший accuracy (причем основные классы - physics, math и cs - модель предсказывает все довольно хорошо, т.е. высокое accuracy не из-за дисбаланса классов), то я решила ее и взять, как итоговую.
